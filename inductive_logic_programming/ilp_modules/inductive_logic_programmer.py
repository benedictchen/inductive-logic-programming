...
# ===============================================================================
# These classes provide backward compatibility with tests that expect specific
# class names and interfaces not present in the main implementation.
...
class InductiveLogicProgramming:
    ...
    def __init__(self, algorithm: str = 'foil', max_clauses: int = 100, 
        ...
        # Create underlying ILP system - filter out unknown kwargs
        test_only_params = [
            'learning_strategy', 'search_strategy', 'predicate_invention', 
            'performance_optimization', 'noise_handling', 'beam_width',
            'use_types', 'use_lookahead', 'clause_selection', 'refinement_operator',
            'max_clause_length', 'max_variables', 'min_accuracy', 'noise_level',  # These are handled separately
            'max_search_depth',  # Search depth parameter
            'allow_predicate_invention', 'max_new_predicates',  # Predicate invention parameters
            'use_pruning', 'use_caching', 'parallel_processing'  # Performance optimization parameters
        ]
        ilp_kwargs = {}
        for key, value in kwargs.items():
            if key not in test_only_params:
                ilp_kwargs[key] = value
        ...
        # Store test parameters for access
        self.learning_strategy = kwargs.get('learning_strategy', 'top_down')
        self.search_strategy = kwargs.get('search_strategy', 'breadth_first') 
        self.predicate_invention = kwargs.get('predicate_invention', False)
        self.performance_optimization = kwargs.get('performance_optimization', True)
        self.noise_handling = kwargs.get('noise_handling', 'default')
        self.beam_width = kwargs.get('beam_width', 10)
        self.use_types = kwargs.get('use_types', False)
        self.use_lookahead = kwargs.get('use_lookahead', True)
        self.clause_selection = kwargs.get('clause_selection', 'best_first')
        self.refinement_operator = kwargs.get('refinement_operator', 'standard')
        self.max_search_depth = kwargs.get('max_search_depth', 5)
        self.allow_predicate_invention = kwargs.get('allow_predicate_invention', False)
        self.max_new_predicates = kwargs.get('max_new_predicates', 3)
        self.use_pruning = kwargs.get('use_pruning', True)
...
# Import algorithm classes from other modules for compatibility
try:
    ...
    class FOILAlgorithm:
        """Fallback FOILAlgorithm class"""
        pass
    
    class ProgolAlgorithm:
        """Fallback ProgolAlgorithm class"""  
        pass